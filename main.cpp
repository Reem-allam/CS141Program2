...
void reveal(int i,int j) {
  ...
  if(!isMine(i, j)) {
    ...
    // Check top
    if (isMine(i-1, j) && (i-1 >= 0)) {
      surroundingMines++;
    }

    // Check bottom
    if (isMine(i+1, j) && (i+1 <= 7)) {
      surroundingMines++;
    }

    // Check left
    if (isMine(i, j-1) && (j-1 >= 0)) {
      surroundingMines++;
    }

    // Check right
    if (isMine(i, j+1) && (j+1 <= 4)) {
      surroundingMines++;
    }

    // Check bottom right
    if (isMine(i+1, j+1) && (i+1 <= 7 && j+1 <= 4)) {
      surroundingMines++;
    }

    // Check bottom left
    if (isMine(i+1, j-1) && (i+1 <= 7 && j-1 >= 0)) {
      surroundingMines++;
    }

    // Check top right
    if (isMine(i-1, j+1) && (i-1 >= 0 && j+1 <= 4)) {
      surroundingMines++;
    }

    // Check top left
    if (isMine(i-1, j-1) && (i-1 >= 0 && j-1 >= 0)) {
      surroundingMines++;
    }

    setCell(i, j, surroundingMines);
  }
}
